...
def upgrade() -> None:
    ...
    # Skip PostGIS extension tables - they're managed by the extension
    # Use IF EXISTS to avoid transaction abort when tables are protected by PostGIS
    # Note: These may fail silently if PostGIS extension owns them, which is expected
    op.execute('DROP TABLE IF EXISTS topology CASCADE')
    op.execute('DROP TABLE IF EXISTS spatial_ref_sys CASCADE')
    op.execute('DROP TABLE IF EXISTS layer CASCADE')
